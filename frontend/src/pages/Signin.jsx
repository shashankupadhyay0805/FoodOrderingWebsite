...
const SignIn = () => {
  ...
  const [error, setError] = useState("");
  const [isLoading, setIsLoading] = useState(false);
  const { login } = useAuth(); // ✅ using login from AuthProvider
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await login(formData.email, formData.password); // ✅ centralized login logic
    setIsLoading(false);
    ...
    <div className="min-h-[calc(100vh-4rem)] flex items-center justify-center pt-32 py-12 px-4 sm:px-6 lg:px-8">
      ...
      >
        <div className="bg-white/80 backdrop-blur-sm rounded-lg shadow-xl p-8 border border-gray-200">
          ...
          <form onSubmit={handleSubmit} classNam
